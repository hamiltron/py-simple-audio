...
class WaveObject(object):
    ...
    def play(self):
        return play_buffer(self.audio_data, self.num_channels, self.bytes_per_sample, self.sample_rate)
    ...
    def from_wave_read(cls, wave_read):
        return cls(wave_read.readframes(wave_read.getnframes()), wave_read.getnchannels(),
                   wave_read.getsampwidth(), wave_read.getframerate())

    def __str__(self):
        return "Wave Object: {} channel, {} bit, {} Hz".format(self.num_channels, self.bytes_per_sample * 8, self.sample_rate)
